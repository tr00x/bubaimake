    ...
    <Link to={`/catalog/${id}`} className="bg-background rounded-2xl border border-border overflow-hidden flex flex-col group cursor-pointer hover:-translate-y-1 transition-all duration-300">
      <div className="relative aspect-[4/3] w-full bg-secondary overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-2">
          {tags.map((t) => (
            <div key={t} className={`bg-background/90 px-2.5 py-1 rounded-md border backdrop-blur-sm flex items-center justify-center ${t === "Горячее" ? "border-red-500" : "border-border"}`}>
              <span className={`text-[11px] font-semibold uppercase tracking-wider ${t === "Горячее" ? "text-red-600" : "text-foreground"}`}>{t}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col gap-4 flex-1">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {meta.map((text) => (
              <span key={text} className="text-[13px] text-muted-foreground bg-secondary px-2 py-1 rounded-md">
                {text}
              </span>
            ))}
          </div>
